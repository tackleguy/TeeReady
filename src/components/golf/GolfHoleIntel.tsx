...
export function GolfHoleIntel({ forecast, turf, miss }: Props) {
  ...
  return (
    <div className="border-t border-[var(--line-subtle)] px-3 py-2.5">
      ...
      {first && (
        <div className="mt-1.5 flex flex-wrap gap-1.5">
          {first.lines.map((line) => (
            <span
              key={line.id}
              className="inline-flex items-center gap-1 rounded-md bg-black/25 px-1.5 py-0.5 text-[11px] text-[var(--ink-2)]"
            >
              <span
      ...
      <ul className="mt-1.5 space-y-0.5 text-[11px] text-[var(--ink-3)]">
